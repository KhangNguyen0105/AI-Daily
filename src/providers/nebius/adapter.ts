...
import { nebiusConfig } from './config';
 ...
 */
export class NebiusAdapter extends ProviderAdapter {
  config = nebiusConfig;
  ...
  async extract(html: string): Promise<ProviderExtraction> {
    try {
      ...
      const { object } = await generateObject({
        ...
        prompt: `Extract Nebius model pricing from this HTML page.
Nebius provides GPU/compute and LLM endpoints.
Extract input and output prices per million tokens and context window size.
Look for region-specific pricing if present.
...
${truncatedHtml}`,
      ...
      console.error('Nebius extraction failed:', error);
      throw error;
    }
  }
