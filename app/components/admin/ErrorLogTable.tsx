  ...
  if (errors.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-sm text-text-tertiary">No errors recorded</p>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border-primary bg-bg-secondary">
            <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wide text-text-tertiary">Time</th>
            <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wide text-text-tertiary">Provider</th>
            <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wide text-text-tertiary">Error</th>
          </tr>
        </thead>
        <tbody>
          {errors.map((errorRun) => {
              ...
              return (
                <Fragment key={key}>
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-accent-red truncate max-w-xs">Pipeline run failed</td>
                  </tr>
                  {isExpanded && (
                    <tr>
                      <td colSpan={3} className="p-0 border-b border-border-primary">
                        <div className="px-4 py-4 bg-bg-secondary/80 shadow-inner">
                          <h4 className="text-xs font-semibold uppercase tracking-wide text-text-tertiary mb-2">Error Details</h4>
                          <pre className="text-xs text-accent-red bg-badge-red-bg p-3 rounded border border-badge-red-border whitespace-pre-wrap overflow-auto max-h-60">
              ...
              return (
                <Fragment key={key}>
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-accent-red truncate max-w-xs">{detail.error}</td>
                  </tr>
                  {isExpanded && (
                    <tr>
                      <td colSpan={3} className="p-0 border-b border-border-primary">
