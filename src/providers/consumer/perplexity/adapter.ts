  ...
  async extract(html: string): Promise<ProviderExtraction> {
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(
          () => reject(new Error('Perplexity consumer extraction timed out')),
      ...
      const extractionPromise = generateObject({
        ...
        prompt: `Extract all consumer subscription plans from this Perplexity pricing page.
...
${truncatedHtml}`,
      ...
      console.error('Perplexity consumer extraction failed:', error);
