  ...
  async extract(html: string): Promise<ProviderExtraction> {
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(
          () => reject(new Error('Copilot consumer extraction timed out')),
      ...
      const extractionPromise = generateObject({
        ...
        prompt: `Extract all consumer subscription plans from this Microsoft Copilot pricing page.
...
${truncatedHtml}`,
      ...
      console.error('Copilot consumer extraction failed:', error);
