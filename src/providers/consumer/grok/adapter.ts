  ...
  async extract(html: string): Promise<ProviderExtraction> {
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(
          () => reject(new Error('Grok consumer extraction timed out')),
      ...
      const extractionPromise = generateObject({
        ...
        prompt: `Extract all consumer subscription plans from this X (Twitter) Premium pricing page.
...
${truncatedHtml}`,
      ...
      console.error('Grok consumer extraction failed:', error);
