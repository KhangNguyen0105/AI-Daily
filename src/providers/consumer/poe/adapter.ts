  ...
  async extract(html: string): Promise<ProviderExtraction> {
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(
          () => reject(new Error('Poe consumer extraction timed out')),
      ...
      const extractionPromise = generateObject({
        ...
        prompt: `Extract all consumer subscription plans from this Poe pricing page.
...
${truncatedHtml}`,
      ...
      console.error('Poe consumer extraction failed:', error);
