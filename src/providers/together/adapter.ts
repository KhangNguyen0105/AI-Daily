...
import { togetherConfig } from './config';
...
export class TogetherAdapter extends ProviderAdapter {
  ...
  async crawl(): Promise<CrawlResult> {
    ...
    if (!result) {
      throw new Error('Failed to crawl Together AI pricing page');
    }
    return result;
  }
  ...
  async extract(html: string): Promise<ExtractionResult[]> {
    try {
      ...
      const { object } = await generateObject({
        ...
        prompt: `Extract Together AI model pricing from this HTML page.
Look for various open-source models with Together pricing.
...
${truncatedHtml}`,
      ...
      console.error('Together AI extraction failed:', error);
      throw error;
    }
  }
