...
export class XAIAdapter extends ProviderAdapter {
  ...
  async crawl(): Promise<CrawlResult> {
    ...
    if (!result) {
      throw new Error('Failed to crawl xAI pricing page');
    }
    return result;
  }
  ...
  async extract(html: string): Promise<ExtractionResult[]> {
    try {
      ...
      const { object } = await generateObject({
        ...
        prompt: `Extract xAI Grok model pricing from this HTML page.
Look for grok-2, grok-2-mini, grok-3, etc.
...
${truncatedHtml}`,
      ...
      console.error('xAI extraction failed:', error);
      throw error;
    }
  }
