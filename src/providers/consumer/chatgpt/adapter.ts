  ...
  async extract(html: string): Promise<ProviderExtraction> {
    try {
      ...
      const extractionPromise = generateObject({
        ...
        prompt: `Extract all consumer subscription plans from this ChatGPT pricing page.
...
${truncatedHtml}`,
      ...
      // Review #1: Cross-check plan names against expectedPlanNames
      ...
      // Review #6: Graceful failure — return empty results on timeout or error
